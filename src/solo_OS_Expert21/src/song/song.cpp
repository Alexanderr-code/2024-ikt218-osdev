extern "C" {
    #include "libc/song.h"
    #include "libc/timer.h"
    #include "libc/common.h"
    #include <libc/stddef.h>
    #include "libc/screen.h"
}

void enable_speaker(){
    // Read the current state of the PC speaker control register
    uint8_t speaker_state = inb(PC_SPEAKER_PORT);
    /*
    Bit 0: Speaker gate
            0: Speaker disabled
            1: Speaker enabled
    Bit 1: Speaker data
            0: Data is not passed to the speaker
            1: Data is passed to the speaker
    */
    // Check if bits 0 and 1 are not set (0 means that the speaker is disabled)
    if ((speaker_state & 3) != 3) {
        // If bits 0 and 1 are not set, enable the speaker by setting bits 0 and 1 to 1
        outb(PC_SPEAKER_PORT, speaker_state | 3);
    }
}

void disable_speaker() {
    // Turn off the PC speaker
    uint8_t speaker_state = inb(PC_SPEAKER_PORT);
    outb(PC_SPEAKER_PORT, speaker_state & 0xFC);
}


void stop_sound() {
    // Step 1: Read the current state from the PC speaker control port
    uint8_t current_state = inb(PC_SPEAKER_PORT);
    
    // Step 2: Clear the bit that enables speaker data to stop the sound
    // Calculate the bitmask to clear the appropriate bit
    uint8_t mask = 0xFE; // 1111 1110 in binary
    
    // Clear the bit using bitwise AND with the complement of the bit responsible for enabling the speaker data
    current_state &= mask;
    
    // Write the modified state back to the control port to stop the sound
    outb(PC_SPEAKER_PORT, current_state);
}

void play_song(Song *song) {
    play_song_impl(song);
}

void play_sound(uint32_t frequency) {
    if (frequency == 0) {
        return;
    }

    int divisor = (uint16_t)(PIT_BASE_FREQUENCY / frequency);

    // Set up the PIT
    outb(PIT_CMD_PORT, 0b10110110); 
    outb(PIT_CHANNEL2_PORT, (uint8_t)(divisor & 0xFF));
    outb(PIT_CHANNEL2_PORT, (uint8_t)(divisor >> 8));

    u8int state = inb(PC_SPEAKER_PORT);
    state |= 0b11;
    outb(PC_SPEAKER_PORT, state);
}

void play_song_impl(Song *song) {
    enable_speaker();
    for (uint32_t i = 0; i < song->length; i++) {
        Note* note = &song->notes[i];
        printf("Note: %s, Freq=%d, Sleep=%d\n", i, note->frequency, note->duration);
        play_sound(note->frequency);
        sleep_interrupt(note->duration);
        disable_speaker();
    }
}

SongPlayer* create_song_player() {
    SongPlayer* player = new SongPlayer();
    player->play_song = play_song_impl;
    return player;
}
